...
### 方法1: ZIPファイルからのインストール（推奨）

**最も簡単な方法** - 事前にビルドされたZIPファイルを使用：

1. **ZIPファイルのダウンロード**
   - `dist.zip` ファイルをダウンロード
   - 任意のフォルダに展開

2. **Chrome拡張機能として読み込み**
   - Chromeブラウザを開く
   - アドレスバーに `chrome://extensions/` と入力してEnter
   - 右上の「デベロッパーモード」トグルスイッチを **ON** にする
   - 「パッケージ化されていない拡張機能を読み込む」ボタンをクリック
   - 展開した `dist` フォルダを選択して「フォルダーの選択」をクリック

3. **動作確認**
   - ブラウザの右上に拡張機能のアイコンが表示されることを確認
   - アイコンをクリックしてポップアップが開くことを確認

### 方法2: ソースコードからのビルド（開発者向け）
...
4. **Chrome拡張機能として読み込み**
   ...
   - 右上の「デベロッパーモード」トグルスイッチを **ON** にする
   - 「パッケージ化されていない拡張機能を読み込む」ボタンをクリック
   - プロジェクトの `dist` フォルダを選択して「フォルダーの選択」をクリック
   - 拡張機能が正常に読み込まれると、拡張機能一覧に表示されます

5. **動作確認**
   - ブラウザの右上に拡張機能のアイコンが表示されることを確認
   - アイコンをクリックしてポップアップが開くことを確認
   - テストページで個人情報のマスキングが機能することを確認

### 注意事項
- **デベロッパーモード**での読み込みは開発・テスト用です
- Chromeを再起動すると「無効な拡張機能」として警告が表示される場合があります
- 一般ユーザーには**方法1のZIPファイル**を使用することを推奨します
- 最終的にはChrome Web Storeでの公開が最もユーザーフレンドリーです

### ZIPファイルの作成方法（開発者向け）

ソースコードから配布用のZIPファイルを作成する場合：
